...
const steps = [
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
        <rect x="2" y="2" width="14" height="14" rx="3" stroke="#00D4A8" strokeWidth="1.4" />
        <path d="M6 9H12M9 6V12" stroke="#00D4A8" strokeWidth="1.4" strokeLinecap="round" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
        <path d="M9 2V9L13 13" stroke="#00D4A8" strokeWidth="1.4" strokeLinecap="round" />
        <circle cx="9" cy="9" r="7" stroke="#00D4A8" strokeWidth="1.4" />
      </svg>
    ),
  },
  {
...
export function HowItWorksSection() {
  ...
  return (
    <section className="bg-lapis py-[72px] px-5 md:py-28 md:px-[60px] relative overflow-hidden">
      ...
      <h2 className="font-serif text-[clamp(36px,4.2vw,58px)] font-medium tracking-[0.01em] leading-[1.1] mb-[18px] text-white">
        ...
        <br />
        in <em className="italic text-teal font-normal">four steps</em>
      </h2>
      ...
      >
        {steps.map((step, i) => (
          <div
            key={step.num}
            className="bg-white/[0.04] border border-white/[0.07] p-7 md:px-7 md:py-8 relative overflow-hidden hover:bg-white/[0.07] transition-colors duration-200"
            style={{ transitionDelay: `${i * 75}ms` }}
          >
            <div className="font-serif text-[80px] font-bold text-white/[0.04] leading-none absolute -bottom-2 right-3.5 pointer-events-none">
              {step.bg}
            </div>
            <div className="font-mono text-[9px] tracking-[0.16em] uppercase text-teal mb-3.5">
              {step.num}
            </div>
            <div className="w-[38px] h-[38px] rounded-lg bg-teal/10 flex items-center justify-center mb-4">
              {step.icon}
            </div>
            <div className="font-serif text-xl font-medium text-white mb-2">{step.title}</div>
            <p className="text-[13px] font-light text-white/40 leading-[1.75]">{step.desc}</p>
          </div>
        ))}
      </div>
    </section>
